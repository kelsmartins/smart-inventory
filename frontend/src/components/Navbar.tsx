...
import { Package, User, Home, HandCoins, LogOut } from 'lucide-react'; // ✅ LogOut centralizado aqui
...
export function Navbar() {
  // ✅ CORREÇÃO: Todos os hooks DEVEM ficar aqui dentro
  ...
  // Esconde a Navbar em rotas específicas
  if (
    pathname === '/welcome' ||
    pathname === '/login' ||
    ...
    <nav className="sticky top-0 z-50 bg-[#222222] h-[60px] text-white">
      <div className="w-full flex h-16 items-center justify-between px-2 sm:px-4">
        ...
        <div className="flex items-center gap-1">
          {links.map(({ to, label, icon: Icon }, index) => (
            <Link
              ...
              className={`flex items-center gap-1.5 rounded-lg px-2 sm:px-3 py-2 text-sm font-medium transition-colors
                ...
                ${to === '/new-sale' ? 'bg-[#6b9dff] hover:bg-[#6b9dff]/70' : ''}
              `}
            >
          ...
          {/* ✅ Botão de Sair posicionado corretamente dentro do JSX */}
          {user && (
            <button
              onClick={logout}
